...
int selectMaxOption(Move options[]);
int selectMinOption(Move options[]);
void saveField();
void restoreField();
int  scoreOption(int player, Move option);
int  scoreDefense(int player, Move option);
int  scoreOffense(int player, Move option);
void printOptions(Move options[]);
...
// store the existing field into a temp array
void saveField() {
	for (int i = 0; i < ROWS; i++) {
		for (int j = 0; j <	COLUMNS; j++) {
...
// restore the existing field from the temp array
void restoreField() {
	for (int i = 0; i < ROWS; i++) {
		for (int j = 0; j <	COLUMNS; j++) {
...
Move *getOptions() {
	...
	for (int i = 0; i < COLUMNS; i++) {
		options[i].col = -1;
		options[i].row = -1;
		// iterate through the column to find empty slot
...
Move selectOption(Move options[]) {
	saveField();
	int bestScore = selectMaxOption(options);
	printf("%d\n", bestScore);
	int bestOption = 1;
	for (int i = 0; i < COLUMNS; i++) {
		if (options[i].col != -1 && options[i].row != -1) {
			makeMove(2, options[i]);
			if (selectMaxOption(getOptions()) == bestScore) {
				bestOption = i;
			}
			clearMove(options[i]);
		}
...
int selectMaxOption(Move options[]) {
	int score = MIN_INT;
	for (int i = 0; i < COLUMNS; i++) {
		// only iterate through valid options
		if (options[i].col != -1 && options[i].row != -1) {
			// base case
			if (checkWinner(2, options[i])) {
				return scoreOption(2,options[i]);
			}
			// recursive case
			else {	
				makeMove(2, options[i]);
				int tempScore = selectMinOption(getOptions());
				if (tempScore > score) {
					score = tempScore;
				}
			}
...
int selectMinOption(Move options[]) {
	...
	for (int i = 0; i < COLUMNS; i++) {
		...
		if (options[i].col != -1 && options[i].row != -1) {
			// base case
			if (checkWinner(1, options[i])) {
				return scoreOption(1,options[i]);
			}
			// recursive case
			else {	
				makeMove(1, options[i]);
				int tempScore = selectMaxOption(getOptions());
				if (tempScore < score) {
					score = tempScore;
				}
			}
...
int scoreOption(int player, Move option) {
	return scoreDefense(player, option) + scoreOffense(player, option);
}
...
int scoreDefense(int player, Move option) {
	...
	field[option.row][option.col] = token;
	// slightly prefer middle columns by default
	int score = (COLUMNS / 2) - abs(option.col - (COLUMNS/2));
...
int scoreOffense(int player, Move option) {
	...
	for (int i = WINTHRESH; i > 0; i--) {
		...
		if (i == WINTHRESH && score >= WINTHRESH/2) {
			score *= score;
		}
	}
